 ...
 *
 * call kanJiang(29, 0, '2,3,0,1,5');
 * call kanJiang(11, 0, '3,5,0,0,4');
 */
drop procedure if exists kanJiang;
delimiter $
create procedure kanJiang(_betId int, _zjCount int, _kjData varchar(255) character set utf8)
begin
 ...
 *清数据
  *
 */
-- call clearData('2012-09-05');

drop procedure if exists clearData;
delimiter $
create procedure clearData(dateString varchar(255)) begin
...
-- select b.uid, b.username, sum(b.`mode` * b.actionNum * b.beiShu) tjAmount from ssc_bets b where b.kjTime>=1347033600 and b.uid not in(select distinct l.extfield0 from ssc_coin_log l where liqType=53 and l.actionTime>=1347033600) group by b.uid having tjAmount>=500;
...
create procedure cancelBet(_zhuiHao varchar(255)) begin
	...
	declare done int default 0;
	declare cur cursor for
	select id, `mode` * beiShu * actionNum, `uid`, `type` from ssc_bets where serializeId=_zhuiHao and lotteryNo='' and isDelete=0;
	declare continue HANDLER for not found set done=1;
...
create procedure pro_pay() begin
	...
	declare info varchar(64) character set utf8 default '自动充值';
	declare done int default 0;
	
	declare isFirstRecharge int;
	
	declare cur cursor for
	select m.id, m.addmoney, m.o_time, m.u_id, m.memo,		u.`uid`, u.coin, u.fcoin,		r.id, r.amount from ssc_members u, my18_pay m, ssc_member_recharge r where u.`uid`=r.`uid` and r.rechargeId=m.u_id and m.`state`=0 and r.`state`=0 and r.isDelete=0;
	declare continue HANDLER for not found set done = 1;

	start transaction;
		open cur;
			repeat
				fetch cur into _m_id, _addmoney, _rechargeTime, _rechargeId, _info, _uid, _coin, _fcoin, _r_id, _amount;
				
				if not done then
					-- select _r_id;
					-- if _amount=_addmoney then
						call setCoin(_addmoney, 0, _uid, _liqType, 0, info, _r_id, _rechargeId, '');
						update ssc_member_recharge set rechargeAmount=_addmoney, coin=_coin, fcoin=_fcoin, rechargeTime=currentTime, `state`=2 where id=_r_id;
...
create procedure pro_count(_date varchar(20)) begin
	...
	select `type`, playedId, _date, sum(actionNum), sum(actionNum * beiShu * `mode`), sum(bonus) from ssc_bets where kjTime between fromTime and toTime and isDelete=0 group by type, playedId
...
-- 重新统计
insert into ssc_count(`type`, playedId, `date`, betCount, betAmount, zjAmount)
select `type`, playedId, date_format(from_unixtime(kjTime), '%Y-%m-%d') `date`, sum(actionNum), sum(actionNum * beiShu * `mode`), sum(bonus) from ssc_bets where isDelete=0 group by `type`, playedId, `date`
on duplicate key update betCount=values(betCount), betAmount=values(betAmount), zjAmount=values(zjAmount);
