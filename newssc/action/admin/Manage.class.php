...
class Manage extends AdminBase{
	...
	public final function addManager(){
		...
		//throw new Exception($this->getValue("select uid from {$this->prename}members where username=?", $para['username']));
	...
	public final function deleteManager($uid){
		...
		$sql="update {$this->prename}members set isDelete=1 where uid=$uid";
		if($this->delete($sql)){
			$manageName=$this->getValue("select username from {$this->prename}members where uid=?", $uid);
	...
	public final function changeManagerPwd($uid){
		if(!$uid=intval($uid)) throw new Exception('参数出错');
		$manageData=$this->getRow("select password, username from {$this->prename}members where uid=$uid");
