...
class AdminBase extends Object{
	private $memberSessionName='member-session-name';
	private $user;
	public $headers;
	...
	public function getDateCount($date=null){
		...
		//$sql="select count(*) betCount, sum(beiShu*mode*actionNum) betAmount, sum(beiShu*mode/2*zjCount*bonusProp) zjAmount from {$this->prename}bets where kjTime between $date and $date+24*3600 and lotteryNo<>'' and isDelete=0";
		$sql="select count(*) betCount, sum(beiShu*mode*actionNum) betAmount, sum(bonus) zjAmount from {$this->prename}bets where kjTime between $date and $date+24*3600 and lotteryNo<>'' and isDelete=0";
		$all=$this->getRow($sql);
