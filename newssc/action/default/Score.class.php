...
class Score extends WebLoginBase{
	...
	public final function viewRule(){
		$this->getSystemSettings();
		$this->display('score/modal-score.php');
	}
	...
	public final function goods($scoretype=null,$limittype=null){
		...
		$sql="select * from {$this->prename}score_goods where startTime<={$this->time} and ";
		switch($this->scoretype){
			case 'current':
				// 正在进行的活动
				switch($this->limittype){
					case 'all':
						//$sql.="(stopTime>{$this->time} or (sum>0 and surplus>0 and stopTime=0) or stopTime=0)";
	...
	public final function swapGood(){
		if(!$para=$_POST) throw new Exception('请求出错');
		if(!$para['goodId']=intval($para['goodId'])) throw new Exception('请求出错');
		...
		try{
			...
			if($good['sum']>0 && $good['surplus']==0) throw new Exception('这礼品已经兑换完了');
			...
			if($good['sum']>0){
				// 限量兑换的礼品
				$sql="update {$this->prename}score_goods set surplus=surplus-1 where id=?";
