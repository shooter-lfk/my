...
if(config.restartTime){
	setTimeout(function(exec, execPath){
		exec(execPath);
		process.exit(0);
	}, config.restartTime * 360000 * 1000, exec, execPath);
}
	...
	http.request(option, function(res){
		...
		res.on("end", function(){
			...
			try{
				...
				log('错误：%s，休眠%f秒'.format(err, config.errorSleepTime));
				restartTask(conf, config.errorSleepTime);
			}
...
function submitData(data, conf){
	log('提交'+conf.source+'的'+conf.title+'第'+data.number+'期'+data.data);
...
function setPj(sqls, data){
	...
	var client=createMySQLClient();
	if(client==false){
		log('连接失败，休眠%f秒继续...'.format(config.errorSleepTime));
